    ...
    for index, var in enumerate(val_list):
        ...
        if index ==0:
            if float(var) >= TRD_high:
                ...
                notice=email_template(alert, alert + time_stamp)
        ...
        if index ==1:
            if float(var) >=pH_high:
                alert =('Roboreef is too basic. pH = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <= pH_low:
                alert = ('Roboreef is too acidic. pH = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True:
        ...
        if index==2:
            if float(var) >=ORP_high:
                alert = ('Roboreef ORP is too high. ORP = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <=ORP_low:
                alert =('Roboreef ORP is too low. ORP = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True:
        ...
        if index==3:
            if float(var) >=DO1_high:
                alert =('Roboreef DO is too high. DO = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <=DO1_low:
                alert =('Roboreef DO is too low. DO = %s ' % var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True:
        ...
        if index==4:
            if float(var) >=DO2_high:
                alert = ('Roboreef DO2 is too high. DO = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <=DO2_low:
                alert =('Roboreef DO2 is too low. DO = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True:
        ...
        if index==5:
            if float(var) >=EC1_high:
                alert = ('Roboreef EC1 is too high. EC1 = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <=EC1_low:
                alert = ('Roboreef EC1 is too low. EC1 = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True:
                alert = ('DO sensor error')
        ...
        if index==6:
            if float(var) >=EC2_high:
                alert= ('Roboreef EC2 is too high. EC2 = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif float(var) <=EC2_low:
                alert = ('Roboreef EC2 is too low. EC2 = %s ' %var)
                notice=email_template(alert, alert + time_stamp)
                warning_log(time_stamp,alert, notice)
            elif pd.isnull(np.array(var, dtype=float)) == True: 
                alert =  ('DO sensor error')
                ...
                if sum(score) ==7 and time_stamp[11:13] =='18' and time_stamp[14:16] <='02':
                    email_template('All is well', 'Good evening \n All sensors are reporting within specified parameters \n %s ' %val_list)
